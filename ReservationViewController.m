...
@interface ReservationViewController (){
    NSString *account;
    NSMutableArray *reservationImage;
    NSMutableArray *reservationName;
...
- (void)viewDidLoad
{
    ...
    //SQL資料庫
    reservationImage=[[NSMutableArray alloc]init];
    reservationName=[[NSMutableArray alloc]init];
    //POST
    /****************************************************************************************/
    //會員與新會員統計
    ...
    NSURL *connection = [[NSURL alloc] initWithString:@"http://chis.slib.antslab.tw/api/profile/get_reservation_book"];
    //宣告要post的值
    ...
    NSLog(@"reservation=%@",returnvaluearray);
    for(int i=0;i<returnvaluearray.count;i++){  //個人書籍資料存放
        ...
        [reservationImage addObject:borrowrecordURL];
        [reservationName addObject:search[@"name"]];
    }
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    ...
    return reservationName.count;
}
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ReservationCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:@"reservationcell" forIndexPath:indexPath];
    
    [[cell reservationImage]setImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[reservationImage objectAtIndex:indexPath.item]]]]];
    [[cell reservationLable]setText:[reservationName objectAtIndex:indexPath.item]];
