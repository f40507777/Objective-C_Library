...
@interface FavoriteViewController (){
    NSString *account;
    NSMutableArray *favoriteImage;
    NSMutableArray *favoriteName;
    NSString *favoritedURL;
}
...
- (void)viewDidLoad
{
    ...
    //SQL資料庫
    favoriteImage=[[NSMutableArray alloc]init];
    favoriteName=[[NSMutableArray alloc]init];
    //POST
    /****************************************************************************************/
    //會員與新會員統計
    ...
    NSURL *connection = [[NSURL alloc] initWithString:@"http://chis.slib.antslab.tw/api/profile/get_favorite_book"];
    //宣告要post的值
    ...
    for(int i=0;i<returnvaluearray.count;i++){  //個人書籍資料存放
        ...
        favoritedURL=[NSString stringWithFormat:@"http://chis.slib.antslab.tw/api/ebook/get_book_cover/%@",search[@"file_id"]];
        [favoriteImage addObject:favoritedURL];
        [favoriteName addObject:search[@"name"]];
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[cell favoriteImage]setImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[favoriteImage objectAtIndex:indexPath.item]]]]];
    [[cell favoriteLable]setText:[favoriteName objectAtIndex:indexPath.item]];
